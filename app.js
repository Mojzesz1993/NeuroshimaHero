...
// ─── FIREBASE CONFIGURATION (Public Demo Firestore) ───
...
let mpState = {
    ...
    isConnected: false,
    isEditable: true,         // Czy gracz ma włączoną edycję swojej karty
    inspectingPlayerId: null, // null oznacza własną postać
    unsubscribe: null,
...
function connectToSession() {
    if (!isFirebaseInitialized) {
        showToast('✗ Brak biblioteki Firebase. Tryb offline.', 4000);
        return;
    }
    ...
    $('#btn-toggle-edit').classList.remove('hidden'); // Pokaż przycisk blokady/edycji
...
function inspectPlayer(targetPlayerId, targetPlayerName) {
    ...
    // Schowaj przycisk blokady edycji podczas podglądu kogoś innego
...
function switchToMyCharacter() {
    ...
    // Przywróć przycisk blokady edycji
